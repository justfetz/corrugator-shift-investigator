...
def test_reproducible_three_shifts_and_known_downtime(analysis):
    ...
    for shift in (1, 2, 3):
        assert analysis.get_shift_kpis(shift)["rows"][0]["valid_setups"] == len(ET.fromstring(generate())[shift-1].findall('setup'))
    rows = analysis.get_downtime_breakdown(2)["rows"]
    assert rows[0]["category"] == "Knife jam"
    assert rows[0]["down_minutes"] == 12
    assert rows[0]["shift_percent"] == pytest.approx(100*12/480)
...
def test_area_and_speed_independent_calculation(analysis):
    ...
    assert row["dry_end_pct"] == pytest.approx(sum(float(s.get('reject_sheets'))*float(s[0].get('width_in'))*float(s[0].get('length_in'))/144 for s in setups)/area*100)
    assert len(row["source_ids"]) == len(setups)

def test_continuing_order_and_shared_footage():
    setups = ET.fromstring(generate())[0].findall("setup")
    assert any(a[0].get('order_id')==b[0].get('order_id') for a,b in zip(setups,setups[1:]))
    ...
    for setup in setups:
        ...
        assert float(setup.get("gross_feet")) == pytest.approx(travel[0]+float(setup.get('shear_sqft'))*12/float(setup.get('width_in')))
...
def test_corrupt_trim_preserved_and_excluded():
    ...
    try:
        ...
        assert result["rows"][0]["valid_setups"] == len(root[0].findall('setup'))-1
...
def test_tool_arguments_and_bound_values(analysis):
    ...
    assert all(row['category'] != 'Missed splice' for row in result['rows'])
...
def test_chart_uses_rows_and_escapes_labels(analysis):
    ...
    assert '12 min' in text and '4 min' in text
...
def test_wet_end_grade_consistency():
    root=ET.fromstring(generate())
    setups=root[0].findall('setup')
    a,b=next((a,b) for a,b in zip(setups,setups[1:]) if a.get('wet_end_id')==b.get('wet_end_id'))
    b.set('grade','26-C' if a.get('grade')!='26-C' else '32-C')
    b.set('target_fpm','900')
    obj=Analysis(ET.tostring(root))
    try:
        assert any('changes' in r['error'] or 'changed' in r['error'] for r in obj.errors)
