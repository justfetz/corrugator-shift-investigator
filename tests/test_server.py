...
def test_sessions_and_day_scope():
    app=Workbench()
    try:
        first=app.ask({'day':'2026-09-01','shift':2,'question':'Show downtime'})
        other=app.ask({'day':'2026-09-02','shift':3,'question':'Show speed'})
        assert first['session_id']!=other['session_id']
        follow=app.ask({'day':'2026-09-01','shift':2,'question':'Compare that with first shift','session_id':first['session_id']})
