...
finish returns successful evidence IDs. The application formats the numerical answer from those tool results. cannot_answer stops unsupported requests. The model cannot create arbitrary SQL, supply chart rows, read keys or send reports. See [official function-calling guide](https://developers.openai.com/api/docs/guides/function-calling).
